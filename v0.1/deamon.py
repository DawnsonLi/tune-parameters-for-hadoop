...
#url和用户名、密码一定要符合权限
#http://s18:19888/ws/v1/history/mapreduce/jobs/
...
def getJobname(jobid):
    try:
        ...
        print "network error when get parameters for jobid when trying to get the job name:",jobid
    try:
        db = MySQLdb.connect("localhost","root","root","hadoop" )
        sql = "insert into jobsubmit(jobid,jobname) values ("+str(jobid)+","+str(name)+");"
        print sql
        cursor = db.cursor()
        cursor.execute(sql)
        db.commit()
...
def GetsAndSave(start):
   ...
   for jobid in jobid_list:   
       if jobIsSuccessed(jobid) and jobExist(jobid) != True:#不存在则插入数据库
...
def save(jobid):
    db = MySQLdb.connect("localhost","root","root","hadoop" )
    sql = "insert into jobsubmit(jobid,jobname) values "
    name = jobname(jobid)
    print name,"**************"
    l = []
